...
// HOW TO USE: Copy ALL of this into a file named index.js
// ============================================================
...
const SCHOOL_HOLIDAYS_2026 = [
  // Group B (KL, Selangor, Penang, Johor, Sabah, Sarawak, etc.)
...
const PUBLIC_HOLIDAYS_2026 = [
  { name: "Tahun Baharu",                   date: "2026-01-01", type: "federal" },
  { name: "Tahun Baharu Cina",              date: "2026-02-17", type: "federal" },
  { name: "Tahun Baharu Cina (Hari Kedua)", date: "2026-02-18", type: "federal" },
  { name: "Hari Raya Aidilfitri",           date: "2026-03-21", type: "federal", note: "Subject to change" },
  { name: "Hari Raya Aidilfitri (Hari Kedua)", date: "2026-03-22", type: "federal", note: "Subject to change" },
  { name: "Hari Pekerja",                   date: "2026-05-01", type: "federal" },
  { name: "Hari Raya Aidiladha",            date: "2026-05-27", type: "federal", note: "Subject to change" },
  { name: "Hari Raya Aidiladha (Hari Kedua)", date: "2026-05-28", type: "federal", note: "Subject to change" },
  { name: "Hari Wesak",                     date: "2026-05-31", type: "federal" },
  { name: "Hari Keputeraan Agong",          date: "2026-06-01", type: "federal" },
  { name: "Awal Muharam (Maal Hijrah)",     date: "2026-06-17", type: "federal" },
  { name: "Maulidur Rasul",                 date: "2026-08-25", type: "federal" },
  { name: "Hari Kebangsaan",                date: "2026-08-31", type: "federal" },
  { name: "Hari Malaysia",                  date: "2026-09-16", type: "federal" },
  { name: "Hari Deepavali",                 date: "2026-11-08", type: "federal", note: "Subject to change" },
  { name: "Hari Krismas",                   date: "2026-12-25", type: "federal" },
];
...
// ============================================================
// STEP 1 — Generate trends via Claude API
// ============================================================
...
async function generateTrends(weekLabel, upcomingEvents) {
  const eventsContext = upcomingEvents.map(e =>
    e.kind === "public_holiday"
      ...
      : `- School ${e.name} (Group ${e.group || "B"}): ${e.start} to ${e.end} (${e.daysAway} days away)`
  ).join("\n");
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `Today is ${today}. You are a Malaysian travel trends analyst.
...
{
  "summary": "2-sentence summary of this week's travel mood",
  ...
  return {
    ...
    card: {
      ...
      elements: [
        ...
        {
          tag: "div",
          text: { tag: "lark_md", content: `**This week's travel mood**\n${data.summary}` }
        },
        { tag: "hr" },
        // KPIs
        {
        ...
        {
          tag: "div",
          text: { tag: "lark_md", content: "**📍 Trending destinations**" }
        },
        {
          tag: "column_set",
          flex_mode: "stretch",
          columns: [
            {
              ...
              elements: [{
                tag: "div",
                text: { tag: "lark_md", content: `🏠 **Domestic** (${data.domesticShare}%)\n\n${domRows}` }
              }]
            },
            {
              tag: "column", width: "weighted", weight: 1,
              elements: [{
                tag: "div",
                text: { tag: "lark_md", content: `✈️ **International** (${data.internationalShare}%)\n\n${intlRows}` }
              }]
            }
          ]
        },
        ...
        {
          tag: "div",
          text: { tag: "lark_md", content: "**🎡 Trending attractions & activities**" }
        },
        {
          tag: "column_set",
          flex_mode: "stretch",
          columns: [
            {
              tag: "column", width: "weighted", weight: 1,
              elements: [{ tag: "div", text: { tag: "lark_md", content: domActs } }]
            },
            {
              tag: "column", width: "weighted", weight: 1,
              elements: [{ tag: "div", text: { tag: "lark_md", content: intlActs } }]
            }
          ]
        },
        { tag: "hr" },
        ...
        {
          tag: "div",
          text: { tag: "lark_md", content: `**📆 Upcoming demand drivers**\n${eventLines}` }
        },
        { tag: "hr" },
        // Keywords
        {
          tag: "div",
          text: { tag: "lark_md", content: `**🔥 Hot search keywords**\n${keywords}` }
        },
        { tag: "hr" },
        // Sources
        {
        ...
        // Footer
        {
          ...
          elements: [{
            tag: "plain_text",
            content: `Auto-generated by MY Travel Trends Bot · Week of ${weekLabel} · Sent every Monday 9:00 AM MYT`
          }]
        }
      ]
    }
...
async function main() {
  ...
  console.log(`📅 Generating report for week: ${weekLabel}`);
  ...
  for (const webhook of WEBHOOKS) {
    try {
      ...
      console.error(`❌ Failed to send to ${webhook.slice(0, 60)}:`, e.message);
    }
  }
